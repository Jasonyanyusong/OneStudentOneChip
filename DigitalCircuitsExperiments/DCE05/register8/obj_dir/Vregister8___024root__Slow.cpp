// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design implementation internals
// See Vregister8.h for the primary calling header

#include "Vregister8___024root.h"
#include "Vregister8__Syms.h"

//==========


void Vregister8___024root___ctor_var_reset(Vregister8___024root* vlSelf);

Vregister8___024root::Vregister8___024root(const char* _vcname__)
    : VerilatedModule(_vcname__)
 {
    // Reset structure values
    Vregister8___024root___ctor_var_reset(this);
}

void Vregister8___024root::__Vconfigure(Vregister8__Syms* _vlSymsp, bool first) {
    if (false && first) {}  // Prevent unused
    this->vlSymsp = _vlSymsp;
}

Vregister8___024root::~Vregister8___024root() {
}

void Vregister8___024root___eval_initial(Vregister8___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vregister8__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vregister8___024root___eval_initial\n"); );
    // Body
    vlSelf->__Vclklast__TOP__clk = vlSelf->clk;
}

void Vregister8___024root___eval_settle(Vregister8___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vregister8__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vregister8___024root___eval_settle\n"); );
}

void Vregister8___024root___final(Vregister8___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vregister8__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vregister8___024root___final\n"); );
}

void Vregister8___024root___ctor_var_reset(Vregister8___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vregister8__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vregister8___024root___ctor_var_reset\n"); );
    // Body
    vlSelf->load = VL_RAND_RESET_I(1);
    vlSelf->clr = VL_RAND_RESET_I(1);
    vlSelf->clk = VL_RAND_RESET_I(1);
    vlSelf->inp = VL_RAND_RESET_I(8);
    vlSelf->q = VL_RAND_RESET_I(8);
}
