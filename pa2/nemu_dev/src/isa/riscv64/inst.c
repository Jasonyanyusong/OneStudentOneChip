...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 32) & 0xFFFF);
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm,  8) & 0xF);
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 16) & 0xFF);
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm,  8, src2 | (Mr(src1 + imm,  8) & ~Mr(0xf  ,  8))));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sh     , S, Mw(src1 + imm, 16, src2 | (Mr(src1 + imm, 16) & ~Mr(0xff , 16))));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sw     , S, Mw(src1 + imm, 32, src2 | (Mr(src1 + imm, 32) & ~Mr(0xfff, 32))));
