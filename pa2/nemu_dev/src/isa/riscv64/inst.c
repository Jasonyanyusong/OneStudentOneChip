...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, printf("RV64I LWU\n")     , R(rd) = Mr(src1 + imm, 2) & 0xFFFFFFFF);
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , R, printf("RV64I SLLI\n")    , R(rd) = src1 << BITS(imm, 5, 0));
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , R, printf("RV64I SRLI\n")    , R(rd) = src1 >> BITS(imm, 5, 0));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , R, printf("RV64I SRAI\n")    , R(rd) = BITS(src1, 63, 63) << 63 | src1 >> BITS(imm, 5, 0));
